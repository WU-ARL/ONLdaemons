...
then
  ...
  VLAN=$4       # 
  TABLE=$5
  echo "ip route add table $TABLE ${PREFIX}/${MASK} dev $DEV.$VLAN"
  ip route add table $TABLE ${PREFIX}/${MASK} dev $DEV.$VLAN
  RTN=$?
  if [ $RTN -ne 0 ]
  then
...
then
  ...
  then
    echo "no gateway"
    echo "ip route add table $TABLE ${PREFIX}/${MASK} dev $DEV.$VLAN via $GW"
    ip route add table $TABLE ${PREFIX}/${MASK} dev $DEV.$VLAN via $GW
    RTN=$?
    if [ $RTN -ne 0 ]
    then
      echo "Error:ip route add failed $RTN"

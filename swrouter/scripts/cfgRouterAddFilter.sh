...
then
  ...
  if [ "$drop" = "1" ]
  then
    echo "iptables -t mangle -A PREROUTING -i $iface.$vlanNum $protoSpec $dstAddrSpec $srcAddrSpec $dstPortSpec $srcPortSpec $tcpFlagsSpec $ACTION $markSpec"
    sudo  iptables -t mangle -A PREROUTING -i $iface.$vlanNum $protoSpec $dstAddrSpec $srcAddrSpec $dstPortSpec $srcPortSpec $tcpFlagsSpec $ACTION $markSpec
  else
    echo "iptables -t mangle -A PREROUTING -i $iface.$vlanNum $protoSpec $dstAddrSpec $srcAddrSpec $dstPortSpec $srcPortSpec $tcpFlagsSpec $ACTION $markSpec"
    sudo  iptables -t mangle -A PREROUTING -i $iface.$vlanNum $protoSpec $dstAddrSpec $srcAddrSpec $dstPortSpec $srcPortSpec $tcpFlagsSpec $ACTION $markSpec
    echo "iptables -t filter -A FORWARD    -i $iface.$vlanNum $protoSpec $dstAddrSpec $srcAddrSpec $dstPortSpec $srcPortSpec $tcpFlagsSpec $ACTION $markSpec"
    sudo  iptables -t filter -A FORWARD    -i $iface.$vlanNum $protoSpec $dstAddrSpec $srcAddrSpec $dstPortSpec $srcPortSpec $tcpFlagsSpec $ACTION $markSpec
  fi
  ...
  then
    ...
    dstAddrSpec="--dst ${dstAddr}/${dstMask}"
  echo "ip route add table $routeTable unicast ${dstAddr}/${dstMask}  dev ${DEV}.${VLAN} $GW_SPEC"
  sudo  ip route add table $routeTable unicast ${dstAddr}/${dstMask}  dev ${DEV}.${VLAN} $GW_SPEC

  # Add ip rule to direct marked packets to route table
  echo "ip rule add fwmark $filterId table $routeTable"
  sudo  ip rule add fwmark $filterId table $routeTable

  # add a tc rule to direct to queue identified by qid
  echo "tc filter add dev ${DEV}.${VLAN} parent ${outPortNum}:0 protocol ip prio 1 handle $filterId fw flowid ${outPortNum}:$qid"
  sudo  tc filter add dev ${DEV}.${VLAN} parent ${outPortNum}:0 protocol ip prio 1 handle $filterId fw flowid ${outPortNum}:$qid

...
{
  expInfo = ei;
  write_log("session::session: added session " + ei.getID() + 
						" for user " + ei.getUserName());
}
...
vm_ptr
session::addVM(component& c, std::string eaddr, 
							 uint32_t crs, uint32_t mem, 
							 std::string pw, std::string img)
{

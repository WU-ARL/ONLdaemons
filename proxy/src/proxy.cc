/*
 * $Source: /project/arl/arlcvs/cvsroot/wu_arl/openNetLab/proxy/proxy.cc,v $
 * $Author: cgw1 $
...
Manager* Manager::getManager(const std::string& ip, in_port_t pn, int pr)
{
  try {
    ...
    maxsd_(0) 
{
  try {
    ...
    sessCmd[wuctl::sessionCmd::cmdEOF] = NULL;
  ...
  while (status == Running && isOpen()) {
    ...
    try {
      ...
      if (FD_ISSET(lsock_.sd(), &rxSet)) {
        ...
        if (!slock)
          wulog(wulogProxy, wulogError, "Manager::run: unable to lock svecLock!\n");
...
void Manager::do_master(wuctl::sessionCmd::arg_type arg, sRec *rec)
{
  if (arg == wuctl::sessionCmd::nullArg) {
    wulog(wulogProxy, wulogError,
          "Manager::do_master: Arg is null! Ignoring command\n");
    return;
  }
  if (rec == NULL) {
    wulog(wulogProxy, wulogWarn,
          "Manager::do_master: record is null! Ignoring command\n");
    return;
  }

  wulog(wulogProxy, wulogVerb, "Manager::do_master: session %d wants to be slave to %d\n",
        rec->sobj->cid(), arg);

  sRec *master = getSession(arg);
  if(master == NULL) {
    wulog(wulogProxy, wulogVerb, "Manager::do_master: requested master session is null\n");
  ...
  wulog(wulogProxy, wulogVerb, "Manager::do_master: writing slave message to fd %d\n", master->wcfd);
  ...
  wulog(wulogProxy, wulogVerb, "Manager::do_master: session %d sends result %d\n",
...
void Manager::recvFromSess(sRec *rec)
{
  ...
  try {
    ...
    if (cmsg.cmd() == wuctl::sessionCmd::cmdEOF) {
      wulog(wulogProxy, wulogVerb, "Manager::recvFromSess: %d got EOF\n",rec->sobj->cid());
      if (rec->rcfd != -1) {
        remFD(rec->rcfd);
        ::close(rec->rcfd);
        rec->rcfd = -1;
      ...
      case wuctl::hdr_t::getSessions:
        {
          autoLock slock(svecLock_);
          if (!slock)
            wulog(wulogProxy, wulogError, "Manager::doControl: unable to lock svecLock!\n");
...
void Manager::addFD(int newFD)
{
  if (newFD >= maxsd_)
    maxsd_ = newFD + 1;
  FD_SET(newFD, &rset_);
  wulog(wulogProxy, wulogLoud, "Manager::addFD: Added fd %d to rset, new max is %d\n", newFD, maxsd_);
}

void Manager::remFD(int oldFD)
{
  if (oldFD == (maxsd_ + 1))
    resetMaxsd();
  FD_CLR(oldFD, &rset_);
...
void Manager::remSession(sRec &srec)
{
...

  {
    ...
    if (sit == svec_.end()) {
      wulog(wulogProxy, wulogError,
            "Manager::remSession: can\'t locate session object in list of sessions!\n");
    } else {
      svec_.erase(sit);
      if (srec.rcfd != -1) {
        remFD(srec.rcfd);

/*
 * $Source: /project/arl/arlcvs/cvsroot/wu_arl/wusrc/wuPP/msg.h,v $
 * $Author: fredk $
 * $Date: 2007/03/23 21:18:45 $
 * $Revision: 1.20 $
 * $Name:  $
 *
 * File:         msg.h
...
namespace wunet {
  ...
  template <class HDRT> class msgBuf : public wunet::mbuf {
    ...
    public:
      ...
      wupp::errType readmsg(inetSock *sock);
      wupp::errType writemsg(inetSock *sock);
      template <class PHDRT> wupp::errType readPload(inetSock *sock);
      wupp::errType writePload(inetSock *sock);
    ...
    private:
      wupp::errType readHdr(inetSock *);
      wupp::errType readBody(inetSock *);
  ...
  template <class HDRT> wupp::errType msgBuf<HDRT>::readHdr(inetSock *sock)
  {
    ssize_t n;
    wupp::errType stat;
    ...
    n = sock->readv(hvec(), hcnt(), stat);
    if (n == (ssize_t)hlen()) {
      // full header has been read
      hrewind(); 
      plen(hdr().dlen());
      stat = wupp::dataComplete;
    } else if (stat == wupp::dataNone) {
      ...
      hseek(hoffset() + n);
      stat = wupp::dataPartial;
    } else if (stat == wupp::EndOfFile) {
      // no need to change the value of stat
      clear();
    } else if (wupp::isError(stat)) {
      clear();
    } else {
      clear();
  ...
  template <class HDRT> wupp::errType msgBuf<HDRT>::readBody(inetSock *sock)
  {
    wupp::errType stat;
     ssize_t n;
  ...
  template <class HDRT> wupp::errType msgBuf<HDRT>::readmsg(inetSock *sock)
  {
    wupp::errType stat;
#   ifdef WUBUF_DEBUG
      MBUF_CKBUF();
...
#   endif
  ...
  template <class HDRT> wupp::errType msgBuf<HDRT>::writemsg(inetSock *sock)
  {
#   ifdef WUBUF_DEBUG
    MBUF_CKBUF2(hdr().dlen());
#   endif
    ...
    if ((n = sock->writev(mvec(), mcnt(), stat)) == (ssize_t)mlen()) {
      // Done
      rewind();
      st_  = Complete;
      stat = wupp::dataComplete;
...
#   endif
  ...
  template <class HDRT> template <class PHDRT> wupp::errType msgBuf<HDRT>::readPload(inetSock *sock)
  {
#   ifdef WUBUF_DEBUG
      MBUF_CKBUF();
#   endif
  ...
  template <class HDRT> wupp::errType msgBuf<HDRT>::writePload(inetSock *sock)
  {
#   ifdef WUBUF_DEBUG
      MBUF_CKBUF();
#   endif
    ...
    if ((n = sock->writev(pvec(), pcnt(), stat)) == (ssize_t)plen()) {
      prewind();
      st_  = Complete;
      stat = wupp::dataComplete;
    } else if (stat == wupp::dataNone) {

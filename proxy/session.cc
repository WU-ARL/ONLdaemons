...
void Session::do_closeconn(wuctl::sessionCmd::arg_type arg)
{
  ...
  if (validConn(conID) && conID != cid()) {
    ...
    wulog(wulogSession, wulogWarn,
          "Session::do_closeconn(%s): Requested to remove an invalid CID %d\n",
          this->idString().c_str(), conID);
  }
}
...
void Session::do_slave(wuctl::sessionCmd::arg_type arg)
{
  ...
  if (mmsg.writeCmd(this->wcfd()) < 0) {
        wulog(wulogSession, wulogError,
              "Manager::do_slave(%s): Error sending ok response to manager\n",
              this->idString().c_str());
    slave_ = NULL;
  }
  ...
  if(doSend(ctlBuf_) == false) {
      wulog(wulogSession, wulogError,
          "Session::do_slave(%s): Unable to send ok status back to RLI\n",
          this->idString().c_str());
  }
...
void Session::do_masterresp(wuctl::sessionCmd::arg_type arg)
{
  ...
  if (doSend(ctlBuf_) == false) {
      wulog(wulogSession, wulogError,
        "Session::do_masterresp(%s): Unable to send error status back to RLI\n",
        this->idString().c_str());
  }
  status_ = Normal;
  master_ = NULL;
...
try {
  ...
  if (cmsg.readCmd(rcfd()) < 0) {
    wulog(wulogSession, wulogError,
          "Session::cmdHandler(%s): Error reading control pipe\n",
          this->idString().c_str());
    if (rcfd() != -1) {
      FD_CLR(rcfd(), &rset_);
...
void Session::run()
{
  ...
  while (check()) {
    ...
    for (int sd = 0; sd < maxsd_; sd++) {
      ...
      if (conns == NULL) {
        wulog(wulogSession, wulogWarn, "Session::run(%s): cid %d in ready set but conns == NULL\n",
              this->idString().c_str(), sd);
        continue;
      } else if (conns->check() == false) {
        wulog(wulogSession, wulogWarn, "Session::run(%s): cid %d in ready set but conns is not open\n",
            this->idString().c_str(), sd);
        // does nothing if conns == this.
        rmConn(conns);
        continue;
      }
...
#if 0
void *Session::init(void *x)
{
  Session *obj = (Session *)x;
  obj->run();
  delete obj;
  pthread_exit(0);
  return NULL;
...
void Session::do_close()
{
  if (status_ == Slave || status_ == SlavePending) {
    wulog(wulogSession, wulogWarn,
          "Session::do_close(%s): ignoring message (slave)\n", this->idString().c_str());
    return;
  }
  ...
  // No exceptions should be thrown but just in case ...
  wulog(wulogSession, wulogVerb,
        "Session::do_close(%s): closing connection %s\n",
        this->idString().c_str(), conn->idString().c_str());
  if (conn == NULL) {
    wulog(wulogSession, wulogWarn,
          "Session::do_close(%s): Connection %d does not exist\n",
